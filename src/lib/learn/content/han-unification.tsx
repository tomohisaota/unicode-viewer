  ...
  return (
    <>
      ...
      <Section title="論争">
        ...
        <ul className="list-disc pl-6 mt-2 flex flex-col gap-2">
          <li><strong>区別の喪失</strong>: 日本語と中国語の読者は同じコードポイントに対して異なる画形を期待する場合がある。<C>lang</C> 属性とフォントに頼るのは脆弱。</li>
          <li><strong>フォント依存</strong>: 正しい言語タグがなければ、CJK 文字が「間違った」地域形式でレンダリングされ、読者を混乱させる。</li>
          <li><strong>哲学的異議</strong>: 地域変種は十分に分岐しており、単なるグリフ変種ではなく別個の文字であると主張する学者もいる。</li>
        </ul>
        <p className="mt-3">擁護側の反論:</p>
        <ul className="list-disc pl-6 mt-2 flex flex-col gap-2">
          <li><strong>先例</strong>: ラテン文字の &lsquo;a&rsquo; もフォントによって異なる形で描画される（セリフ vs サンセリフ）が、別のコードポイントは与えられない。地域的な CJK グリフ変種も同様。</li>
          <li><strong>実用性</strong>: 統合なしでは CJK ブロックは3〜4倍の大きさになり、相互運用性が困難に。</li>
          <li><strong>IVS という安全弁</strong>: 異体字シーケンス（IVS）により、必要時に正確なグリフ形式を指定可能。</li>
        </ul>
      </Section>
