  ...
  return (
    <>
      ...
      <Section title="実際の攻撃事例">
        ...
        <ul className="list-disc pl-6 mt-2 flex flex-col gap-2">
          <li><strong>IDN ホモグラフ攻撃</strong>: <C>аpple.com</C>（キリル文字のа）のようなドメインを登録し、ブラウザのアドレスバーで <C>apple.com</C> と同一に表示させる手法。これがブラウザにおける IDN 表示ポリシー策定のきっかけとなりました。</li>
          <li><strong>ソースコード攻撃（Trojan Source）</strong>: ソースコードに Bidi オーバーライド文字やホモグリフを挿入し、悪意あるロジックをコードレビュー時に無害に見せる手法。2021 年のケンブリッジ大学の論文が、この手法で人間のレビュアーに見えない脆弱性を注入できることを実証しました。</li>
          <li><strong>フィッシングメール</strong>: 視覚的な検査をパスする混合スクリプトホモグリフを使って送信者名や URL を偽装する手口です。</li>
          <li><strong>SNS のなりすまし</strong>: キリル文字やギリシャ文字の置換を使用して、正規アカウントと同一に見えるユーザー名を作成する手口です。</li>
        </ul>
...
mixed.codePointAt(0)  // 1072 (U+0430 Cyrillic Small Letter A)`}</CodeBlock>
      ...
      <Section title="検出方法">
        ...
        <ul className="list-disc pl-6 mt-2 flex flex-col gap-2">
          <li><strong>スクリプト混在検出</strong>: 複数のスクリプトの文字を含む文字列（例: ラテン文字とキリル文字の混在）をフラグ付けします。Unicode TR#39 は「混合スクリプト」検出アルゴリズムを定義しています。</li>
          <li><strong>紛らわしい文字の検出</strong>: Unicode TR#39 は各文字をその「スケルトン」&mdash; 比較用の正規形 &mdash; にマッピングする <C>confusables.txt</C> も公開しています。同じスケルトンを持つ 2 つの文字列は紛らわしいと判定されます。</li>
          <li><strong>単一スクリプト強制</strong>: 識別子（ユーザー名、ドメイン）が 1 つのスクリプトの文字のみを使用することを要求します。</li>
          <li><strong>視覚的検査ツール</strong>: このツールのように、疑わしいテキストの背後にある実際のコードポイントを明らかにします。</li>
        </ul>
...
export default function HomoglyphsContent({ locale }: { locale: "en" | "ja" }) {
